...
stopuser = {}
token = '6998430537:AAFCp4-wdmFZ6FZLQXghNnhRASK2rTucja8'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/xa_bp")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/xa_bp")
		keyboard.add(contact_button)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='Joooo-'+''.join(random.choices(characters, k=6))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=6))
			current_time = datetime.now()
